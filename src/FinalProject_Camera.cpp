...
int main(int argc, const char *argv[])
{
    ...
    bool verbose = true; // Set to true if log messages like "#2 DETECT & CLASSIFY OBJECTS done" are wanted
    ...
    for (string detectorType : detectors) {
        for (string descriptorType : descriptors) {
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
            {
                ...
                if (!exportFlag) std::cout << "Image " << imgIndex << std::endl;
                ...
                if(bVis)
                {
                    uint imageSize = 800;        // image size for visualization of 3D points
                    show3DObjects((dataBuffer.end()-1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(imageSize, imageSize), true);
                }
                bVis = false;
